...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      const steer = p.followMouse && pointerAngle !== null && (!p.autoAnimate || proximityT > 0.02);
      const target = steer ? pointerAngle! : idleAngle;
      const diff = ((target - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      angle += diff * (1 - Math.exp(-dt * 7));

      const brightTarget = p.autoAnimate ? Math.max(0.65, proximityT) : proximityT;
      bright += (brightTarget - bright) * (1 - Math.exp(-dt * 8));

      button.style.setProperty('--sb-angle', `${angle}rad`);
      button.style.setProperty('--sb-bright', String(Math.min(1.35, bright * p.intensity)));
