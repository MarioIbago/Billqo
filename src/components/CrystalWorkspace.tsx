...
export function CrystalWorkspace({ snapshot, connection, user, activeView, onViewChange, onOpenAdd, onOpenBilling, onEditTransaction, onDeleteTransaction, onDeleteAllTransactions, onDeleteFinancialData, onSaveBudget, onSavePreferences, onRefresh, onReconnect, onDisconnect, onSignOut, busy }: CrystalWorkspaceProps) {
  ...
  const movementChipClass = (active: boolean) => `h-8 shrink-0 rounded-full border px-2.5 text-[11px] font-semibold transition active:scale-[0.98] ${active
  ...
  const renderMovements = () => (
    <section className="crystal-screen" aria-labelledby="movements-title">
      ...
      <section className="crystal-panel crystal-filter-panel" style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 10 }}>
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_150px] items-center gap-2.5 max-[360px]:grid-cols-1">
          <div className="crystal-search !min-w-0 flex-1"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Uber, $500, agosto, Comida…" inputMode="search" /></div>
          <label className="crystal-select !min-w-0"><span className="sr-only">Categoría</span><select className="!min-w-0 !w-full" value={movementCategory} onChange={(event) => setMovementCategory(event.target.value)}><option value="all">Todas las categorías</option>{snapshot.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><ChevronDown size={14} /></label>
          <span className="crystal-filter-count hidden shrink-0 sm:inline-flex"><Filter size={14} />{filteredMovements.length} registro(s)</span>
        </div>
        <div className="-mx-1 flex min-w-0 gap-2 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Filtros rápidos de movimientos">
          ...
          {hasMovementFilters && <button type="button" className="h-8 shrink-0 rounded-full px-2.5 text-[11px] font-semibold text-white/35 transition hover:text-white/65" onClick={clearMovementFilters}><X size={13} className="inline-block" /> Limpiar</button>}
        </div>
      </section>
